...
Upon its reception, node A compares its current epoch to the new epoch introduced.
As $A < B$, node A deems this new epoch as the new primary one.
...
\mnnote{TODO: expliquer que l'on peut simplement inverser le renommage pour les identifiants renommés à partir de leur \emph{offset}, qui correspond en fait à leur index dans \emph{renamedIds}}

\mnnote{TODO: expliquer que pour les autres identifiants, on peut revert le renommage en retournant l'identifiant tronqué de son préfixe, si cela satisfait la relation d'ordre avec les identifiants prédécesseur et successeur}

\mnnote{TODO: expliquer que si ce n'est pas le cas, a recours à des tuples réservés au mécanisme de renommage, $MIN\_TUPLE$ et $MAX\_TUPLE$, pour maintenir l'ordre à travers les epochs}
...
\begin{figure}
    \begin{subfigure}{\columnwidth}
        \begin{tikzpicture}
            ...
            to ++(0:4 * \widthletter) node[block, fill=mydarkpurple,
                    label={ [] below:{\color{mydarkpurpleid}\id{i}{B2}{0..4}} }
                ] (S2B) {HELLO};


            \draw[->, thick] (S0A-right) -- node[above, align=center]{\emph{rename}} (S1A-left);
            \draw[dashed, ->, thick, shorten >= 3] (S1B-right.east) -- node[below right, align=center]{\emph{insert "l" at} {\color{mylightorange}\id{i}{B0}{0}\id{m}{B1}{0}}} (S1A-left.west);
            \draw[->, thick] (S1B-right) -- node[below, align=center]{\emph{rename}} (S2B);
    ...
    \begin{subfigure}{\columnwidth}
        \begin{tikzpicture}
            \path
                ...
                to ++(0:4 * \widthletter) node[letter, label=below:{\id{i}{B0}{0}}] (S2A-left) {H}
                to ++(0:\widthletter) node[letter, fill=mydarkorange, label=above:{\color{mydarkorange}\id{i}{B0}{0}\id{f}{\,A0}{0}}] {E}
                to ++(0:\widthletter) node[letter, fill=mylightorange, label=below:{\color{mylightorange}\id{i}{B0}{0}\id{m}{B1}{0}}] {L}
            ...
            \draw[dashed, ->, thick] (S2B.east) -- node[below right, align=center]{\emph{rename}} (S2A-left.west);
        \end{tikzpicture}
    ...
    \begin{subfigure}{\columnwidth}
        \begin{tikzpicture}
            ...
            \draw[->, thick] (S2A-right) -- node[above, align=center]{\emph{rename}} (S3A);
        \end{tikzpicture}
...
\mnnote{TODO: trouver comment illustrer les epochs, et notamment l'epoch courante, au niveau des représentations de la séquence répliquée de la \autoref{fig:revertRenameId}}
...
\mnnote{TODO: Reprendre la description du scénario et de l'implémentation des benchmarks de PaPoC, en modifiant seulement la partie concernant le bot qui effectue le renommage pour expliquer ici qu'on fait varier \emph{nbRenamingBots} en fonction de la simulation}
