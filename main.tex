...
\begin{itemize}
    \item Propose a new \ac{CRDT}: \emph{RenamableLogootSplit}
    \item Combines LogootSplit with a renaming mechanism
    \item Purposes of this mechanism are to reassing shorter identifiers to elements and to aggregate them into fewer blocks in a fully distributed manner
    \item Describe this new \ac{CRDT}'s behavior in centralised settings first in \autoref{sec:renamablelogootsplit-centralised-settings}, for the sake of simplicity
    \item Describe its behavior in distributed settings in \autoref{sec:renamablelogootsplit-distributed-settings}
\end{itemize}

\section{RenamableLogootSplit in centralised settings}
\label{sec:renamablelogootsplit-centralised-settings}

\subsection{\emph{rename} operation}

\subsection{Dealing with concurrent \emph{simple} updates}

\begin{algorithm}
    \caption{Rename identifier}
    \label{alg:renameId}
    \begin{algorithmic}
    \Function{renameId}{$\trm{id}, \trm{renamedIds}, \trm{nId}, \trm{nSeq}$}
        ...
        \If{$\trm{id} < \trm{firstId}$}
            ...
            \State \Return $\trm{renameIdLessThanFirstId}(\trm{id}, \trm{firstId}, \trm{newFirstId})$
        \ElsIf{$\trm{id} \in \trm{renameIds}$}
            \State $\trm{index} \gets \trm{findIndex}(\trm{id}, \trm{renamedIds})$
            \State \Return $\trm{renameIdFromIndex}(\trm{pos}, \trm{nId}, \trm{nSeq}, \trm{index})$
        \ElsIf{$\trm{lastId} < \trm{id}$}
            \State $\trm{newLastId} \gets \trm{new} \ \trm{Id}(\trm{pos}, \trm{nId}, \trm{nSeq}, \trm{length} - 1)$
            \State \Return $\trm{renameIdGreaterThanLastId(\trm{id}, \trm{lastId}, \trm{newLastId})}$
        \Else
            \State \Return $\trm{renameIdFromPredecessorId}(\trm{id}, \trm{renamedIds})$
        \EndIf
    \EndFunction
    \\
    \Function{renameIdFromPredecessorId}{$\trm{id},\trm{renamedIds}$}
        ...
        \If{$\trm{succId.length} + 1 < \trm{id.length}$}
            \State $\trm{offset} \gets \trm{getOffset}(\trm{succId}) - 1$
...
\label{sec:renamablelogootsplit-distributed-settings}
...
\begin{algorithm}
    ...
    \Function{renameIdFromIndex}{$\trm{pos}, \trm{nId}, \trm{nSeq}, \trm{index}$}
        \State \Return $\trm{new Id}(\trm{pos}, \trm{nId}, \trm{nSeq}, \trm{index})$
    \EndFunction
    \\
    \Function{renameIdLessThanFirstId}{$\trm{id}, \trm{firstId}, \trm{newFirstId}$}
        \State $\trm{offset} \gets \trm{getOffset}(\trm{firstId}) - 1$
        ...
        \State $\trm{prefix} \gets \trm{concat}(\trm{predOfNewFirstId}, \trm{MAX\_TUPLE})$
    ...
    \EndFunction
    \\
    \Function{renameIdGreaterThanFirstId}{$\trm{id}, \trm{lastId}, \trm{newLastId}$}
        \State $\trm{prefix} \gets \trm{concat}(\trm{lastId}, \trm{MIN\_TUPLE})$

        \If{$\trm{newLastId} < \trm{lastId}$ \textbf{and} $\trm{isPrefix}(\trm{prefix}, id)$}
            \State $\trm{tail} \gets \trm{getTail}(\trm{id}, \trm{prefix.length})$
