...
As no coordination is enforced between nodes, several may concurrently rename their respective states.
However, the proposed \emph{rename} operation is not commutative with itself.
Nodes encounter a conflict when dealing with several \emph{rename} operations concurrently issued.

To ensure that they eventually converge, nodes have to solve this conflict. Notice that \emph{rename} operations are system operations : they have no impact on the content of the document and have no user intention attached. Nodes may thus solve the conflict by designating collegially with which \emph{rename} operation to proceed. We call the \emph{rename} operation with which nodes continue the \emph{primary} one. Others from the set of concurrent \emph{rename} operations are called the \emph{secondary} ones.
