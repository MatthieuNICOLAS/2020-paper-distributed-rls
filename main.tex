    ...
    \begin{subfigure}{\columnwidth}
        ...
        \begin{tikzpicture}
            \path
                ...
                to ++(0:\widthletter) node[letter, fill=mydarkblue, label=below:{\color{mydarkblueid}\id{i}{A1}{0}}] {H};
    ...
    \begin{subfigure}{\columnwidth}
        ...
        \begin{tikzpicture}
            \path
                ...
                to ++(0:\widthletter) node[block, label=below:{\id{i}{B0}{1..2}}] (S0A-right) {LO}
                to ++(0:5 * \widthletter) node[epoch] (S1A-left) {\epoch{A1}}
                to ++(0:\widthletter) node[block, fill=mydarkblue, label=below:{\color{mydarkblueid}\id{i}{A1}{0..3}}] {HELO};
...
\mnnote{TODO: Expliquer qu'effectuer un renommage est analogue à effectuer un changement de repère.}
To denotes frames of reference, we use \emph{epochs}.
Initally, the replicated sequence starts at the \emph{origin} epoch noted \epoch{0}.
Each \emph{rename} operations introduces a new epoch and enables the sequence to advance to it from the previous one.
...
For example, the \emph{rename} operation described in \autoref{fig:renaming} enables node A to advance its state from \epoch{0} to \epoch{A1}.
...
To this end, this algorithm relies on the \emph{former state}, the element identifiers of the state upon which the \emph{rename} operation was generated.
...
In \autoref{fig:GC-epoch-trees-step-2}, we represent the new states of their \emph{epoch trees} once they each receive the first \emph{rename} operation issued by the other node.
...
Since in that case node B would have to transform operations to apply them to \epoch{B7}, node B has to retain \epoch{A1} and its corresponding \emph{rename} operation.
